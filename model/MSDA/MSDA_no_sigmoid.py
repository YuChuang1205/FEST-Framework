...
class LH_DWT_2D_attation(nn.Module):
    def __init__(self, wave):
        ...
        w = pywt.Wavelet(wave)
        dec_hi = torch.Tensor(w.dec_hi[::-1])  # tensor([ 0.7071, -0.7071])
        dec_lo = torch.Tensor(w.dec_lo[::-1])  # tensor([0.7071, 0.7071])
        w_lh = dec_lo.unsqueeze(0) * dec_hi.unsqueeze(1)  # tensor([[ 0.5000,  0.5000], [-0.5000, -0.5000]])垂直
        self.register_buffer('w_lh', w_lh.unsqueeze(0).unsqueeze(0))  # 前面扩展2维度，成为4维度
...
class LH_DWT_Function_attation(Function):
    @staticmethod
    def forward(ctx, x, w_lh):  # 在静态方法中，带ctx参数而不是self
        x = x.contiguous()
        ctx.save_for_backward(w_lh)
        ctx.shape = x.shape
        dim = x.shape[1]
        x = torch.nn.functional.pad(x, (1, 0, 1, 0))  # 指的是（左填充，右填充，上填充，下填充），其数值代表填充次数
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            dx = dx.transpose(1, 2).reshape(B, -1, H, W)  # torch.Size([4, 192, 128, 128])
            filters = w_lh  # torch.Size([3, 1, 2, 2])
            filters = filters.repeat(C, 1, 1, 1).to(dtype=torch.float16)  # torch.Size([3, 1, 2, 2])
            dx = torch.nn.functional.conv_transpose2d(dx, filters, stride=1, groups=C)
            dx = torch.cat((dx[:, :, :0], dx[:, :, 1:]), dim=2)  # 删除指定的行,删除第0行
            dx = torch.cat((dx[:, :, :, :0], dx[:, :, :, 1:]), dim=3)  # 删除指定的列,删除第0列
        return dx, None
...
class HL_DWT_2D_attation(nn.Module):
    def __init__(self, wave):
        ...
        w_hl = dec_hi.unsqueeze(0) * dec_lo.unsqueeze(1)  # tensor([[ 0.5000, -0.5000], [ 0.5000, -0.5000]])水平

        self.register_buffer('w_hl', w_hl.unsqueeze(0).unsqueeze(0))  # 前面扩展2维度，成为4维度
...
class HL_DWT_Function_attation(Function):
    @staticmethod
    def forward(ctx, x, w_hl):  # 在静态方法中，带ctx参数而不是self
        x = x.contiguous()
...
class HH_DWT_2D_attation(nn.Module):
    def __init__(self, wave):
        ...
        dec_hi = torch.Tensor(w.dec_hi[::-1])  # tensor([ 0.7071, -0.7071])
        dec_lo = torch.Tensor(w.dec_lo[::-1])  # tensor([0.7071, 0.7071])
        w_hh = dec_hi.unsqueeze(0) * dec_hi.unsqueeze(1)  # tensor([[ 0.5000, -0.5000], [-0.5000,  0.5000]])对角线
        self.register_buffer('w_hh', w_hh.unsqueeze(0).unsqueeze(0))  # 前面扩展2维度，成为4维度
...
class HH_DWT_Function_attation(Function):
    @staticmethod
    def forward(ctx, x, w_hh):  # 在静态方法中，带ctx参数而不是self
        x = x.contiguous()
        ...
        x_hh = torch.nn.functional.conv2d(x, w_hh.expand(dim, -1, -1, -1), stride=1, padding=0,
                                          groups=dim)  # 可以直接指定卷积核#groups是一个数，代表分组卷积时分的组数，特别的当groups = in_channel时，就是在做逐层卷积
        x = x_hh
        return x
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            dx = dx.view(B, 1, -1, H, W)
            dx = dx.transpose(1, 2).reshape(B, -1, H, W)  # torch.Size([4, 192, 128, 128])
            filters = w_hh  # torch.Size([3, 1, 2, 2])
            filters = filters.repeat(C, 1, 1, 1).to(dtype=torch.float16)  # torch.Size([3, 1, 2, 2])
            dx = torch.nn.functional.conv_transpose2d(dx, filters, stride=1, groups=C)
            dx = torch.cat((dx[:, :, :0], dx[:, :, 1:]), dim=2)  # 删除指定的行,删除第0行
            dx = torch.cat((dx[:, :, :, :0], dx[:, :, :, 1:]), dim=3)  # 删除指定的列,删除第0列
...
class LL_DWT_2D_attation(nn.Module):
    def __init__(self, wave):
        ...
        dec_hi = torch.Tensor(w.dec_hi[::-1])  # tensor([ 0.7071, -0.7071])
        dec_lo = torch.Tensor(w.dec_lo[::-1])  # tensor([0.7071, 0.7071])
        w_ll = dec_lo.unsqueeze(0) * dec_lo.unsqueeze(1)  # tensor([[0.5000, 0.5000], [0.5000, 0.5000]])
        self.register_buffer('w_ll', w_ll.unsqueeze(0).unsqueeze(0))  # 前面扩展2维度，成为4维度
...
class LL_DWT_Function_attation(Function):
    @staticmethod
    def forward(ctx, x, w_ll):  # 在静态方法中，带ctx参数而不是self
        x = x.contiguous()
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            dx = dx.view(B, 1, -1, H, W)
            dx = dx.transpose(1, 2).reshape(B, -1, H, W)  # torch.Size([4, 192, 128, 128])
            filters = w_ll  # torch.Size([3, 1, 2, 2])
            filters = filters.repeat(C, 1, 1, 1).to(dtype=torch.float16)  # torch.Size([3, 1, 2, 2])
            dx = torch.nn.functional.conv_transpose2d(dx, filters, stride=1, groups=C)
            dx = torch.cat((dx[:, :, :0], dx[:, :, 1:]), dim=2)  # 删除指定的行,删除第0行
            dx = torch.cat((dx[:, :, :, :0], dx[:, :, :, 1:]), dim=3)  # 删除指定的列,删除第0列
...
class FrequencyAttention(nn.Module):  #
    ...
    def __init__(self, in_channel):
        super().__init__()
        # self.HH_reduce = nn.Sequential(
        #     nn.Conv2d(in_channel, 1, kernel_size=3, padding=1, stride=1),
        #     nn.Sigmoid(),
        # )
        self.HH_reduce = SpatialAttention()
        # self.LH_reduce = nn.Sequential(
        #     nn.Conv2d(in_channel, 1, kernel_size=3, padding=1, stride=1),
        #     nn.Sigmoid(),
        # )
        self.LH_reduce = SpatialAttention()
        # self.HL_reduce = nn.Sequential(
        #     nn.Conv2d(in_channel, 1, kernel_size=3, padding=1, stride=1),
        #     nn.Sigmoid(),
        # )
        self.HL_reduce = SpatialAttention()
        # self.LL_reduce = nn.Sequential(
        #     nn.Conv2d(in_channel, 1, kernel_size=3, padding=1, stride=1),
        #     nn.Sigmoid(),
        # )
        ...
        # self.fc =nn.Linear(in_channel*3, in_channel, bias=False)
...
class DWT_2D(nn.Module):
    def __init__(self, wave):
        ...
        w_ll = dec_lo.unsqueeze(0) * dec_lo.unsqueeze(1)  # tensor([[0.5000, 0.5000], [0.5000, 0.5000]])背景
        w_lh = dec_lo.unsqueeze(0) * dec_hi.unsqueeze(1)  # tensor([[ 0.5000,  0.5000], [-0.5000, -0.5000]])垂直
        w_hl = dec_hi.unsqueeze(0) * dec_lo.unsqueeze(1)  # tensor([[ 0.5000, -0.5000], [ 0.5000, -0.5000]])水平
        w_hh = dec_hi.unsqueeze(0) * dec_hi.unsqueeze(1)  # tensor([[ 0.5000, -0.5000], [-0.5000,  0.5000]])对角线
...
class DWT_Function(Function):
    @staticmethod
    def forward(ctx, x, w_ll, w_lh, w_hl, w_hh):  # 在静态方法中，带ctx参数而不是self
        x = x.contiguous()
...
class Hfrequencyfeature(nn.Module):  #
    ...
    def forward(self, x):  # x:torch.Size([8, 16, 512, 512])
        out = self.DWT_2D(x)  # torch.Size([8, 48, 512, 512])
        return out
...
##############################################################################
class RFB_modified(nn.Module):  # 多尺度卷积
...
class RFB_modified_LCL(nn.Module):  # 多尺度卷积
    def __init__(self, in_channel, out_channel):
...
class MSDANet_No_Sigmoid(nn.Module):
    def __init__(self):
        super(MSDANet_No_Sigmoid, self).__init__()
        self.stage = Stage()
